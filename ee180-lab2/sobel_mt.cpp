...
//Define locks
static pthread_mutex_t lockOne = PTHREAD_MUTEX_INITIALIZER;
static pthread_barrier_t barrierTwo;
...
void *runSobelMT(void *ptr)
{
  ...
  //intialize barrier
  pthread_barrier_init(&barrierTwo, NULL, 2);
...
*/
  ...
  while (1) {
    ...
    //Split Matrices in half 
    Mat first_half_src = src(Rect(0, 0, IMG_WIDTH, (IMG_HEIGHT/2)+1));
    Mat second_half_src = src(Rect(0, (IMG_HEIGHT/2)-1, IMG_WIDTH, IMG_HEIGHT/2));
    Mat first_half_gray = img_gray(Rect(0, 0, IMG_WIDTH, (IMG_HEIGHT/2)+1));
    Mat second_half_gray = img_gray(Rect(0, (IMG_HEIGHT/2)-1, IMG_WIDTH, IMG_HEIGHT/2));

    pc_start(&perf_counters);
    if (myID == thread0_id) {
	grayScale(first_half_src, first_half_gray);
    	pthread_barrier_wait(&endSobel);
    } else {
	grayScale(second_half_src, second_half_gray);
    	pthread_barrier_wait(&endSobel);
    }
    pc_stop(&perf_counters);
    ...
    //Split Matrices in half 
    Mat first_half_out = img_sobel(Rect(0, 0, IMG_WIDTH, IMG_HEIGHT/2+1));
    Mat second_half_out = img_sobel(Rect(0, IMG_HEIGHT/2-1, IMG_WIDTH, IMG_HEIGHT/2));

    pc_start(&perf_counters);
    //Parallel Sobel Calc
    if (myID == thread0_id) {
	sobelCalc(first_half_gray, first_half_out);
	pthread_barrier_wait(&endSobel);
    } else {
	sobelCalc(second_half_gray, second_half_out);
	pthread_barrier_wait(&endSobel);
    }
    pc_stop(&perf_counters);
